...
- The keep chain itself (word-gap/leading/trailing silence, cut-list union, `<keep>` subtraction, `min_keep`, margins, caption coverage, `min_keep` enforcement, `min_cut`) lives in `intervals/keep.py::compute_keep_intervals()`; `run_intervals()` wraps it with the marker extraction and the overlay snap. `dropped_ranges()` is the same chain fed what the stage feeds it (edit lines synced, their `<keep>`s honoured, no director op) and returns the complement of the keep intervals — the director's line brackets and summary's part `silence` read it, so a bracket prices exactly what renders. `tests/intervals/test_keep.py` pins the parity against a real `run_intervals()` run.
