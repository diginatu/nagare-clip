...
def test_resegment_rebuilds_and_preserves_text(monkeypatch):
    ...
    monkeypatch.setattr(
        ss_cli, "bunsetsu_units", lambda text, nlp: [(i, i + 1, ch) for i, ch in enumerate(text)]
    )
    monkeypatch.setattr(
        ss_cli, "split_window", lambda bunsetsu, cfg, **kw: [(0, 2), (3, len(bunsetsu) - 1)]
    )
...
def test_resegment_degraded_window_keeps_original(monkeypatch):
    ...
    out = ss_cli.resegment_json(
        data,
        {"enabled": True, "window_segments": 20},
        nlp=None,
        recorder=ss_cli.NULL_RECORDER,
        stem="x",
    )
