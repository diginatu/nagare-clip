...
class TestNextRequest:
    ...
    def test_the_last_chunk_stops_at_the_last_line(self):
        assert "around lines 1 to 10" in next_request(_view(), LoopState(), 40)
...
class TestMalformedReply:
    ...
    def test_a_reply_that_is_not_an_object(self):
        assert apply_reply(_view(), LoopState(), "[1, 2]").error is not None

    def test_a_reply_with_neither_done_nor_range(self):
        assert apply_reply(_view(), LoopState(), '{"ops": []}').error is not None

    def test_a_range_outside_the_video(self):
        assert apply_reply(_view(), LoopState(), _reply((1, 99), 4, [])).error is not None

    def test_a_range_that_runs_backwards(self):
        assert apply_reply(_view(), LoopState(), _reply((4, 1), 4, [])).error is not None

    def test_a_missing_reviewed_through(self):
        reply = json.dumps({"range": [1, 4], "ops": []})
        assert apply_reply(_view(), LoopState(), reply).error is not None
