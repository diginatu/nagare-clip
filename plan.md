...
## Project editorial brief (`project:` section) (2026-07-28)

**Status: complete.** `summary`, `plan`, `director` and `text_filter` could only see the transcript and the artifacts of earlier stages, so three kinds of judgement had no input at all — visible in the `water_pump_3` run (3 sources, 90.8min): the cut opens on 今日の本題はこれ… with no way for the director to know the project continues `water_pump_2`; 7 of 12 `plan.json` directions collapsed to boilerplate `shorten — trim repetitive …` with no target length to judge against; and a brief asking for a fast, punchy vlog produced 4 overlays across 25.8min of finished video, because nothing carried "punchy" into a prompt.

New `project:` config section (`ProjectConfig`, six free-text fields, all `""` by default): `audience`, `purpose`, `target_duration`, `tone`, `story_so_far`, `previous_summary`. `nagare_clip.brief.format_brief()` renders the set fields as a fixed-order `Editorial brief (…):` block; `apply_brief(stage_cfg, cfg, keys=…)` returns a copy of a stage's config with the block appended to its prompt key(s). Wiring lives in the four stages' `run.py` — `summary` briefs both `prompt` and `overall_prompt` (map and reduce), `director` puts the brief ahead of the summary/plan overview block, `text_filter` puts it ahead of `build_enhanced_prompt`'s per-video context so the transcript-specific context stays last. No LLM module gained a parameter; the brief travels in the already-merged `cfg` dict.
...
## "keep" disambiguated: plan vocabulary vs. director op (2026-07-29)

**Status: complete.** `keep` meant two unrelated things one stage apart: in `PLAN_PROMPT` it was an editorial direction verb ("this part earns its place"), in `director` it is an op that protects a span from cutting **including every silent second inside it**. Since `plan.json` is fed to the director as context, the word was copied straight across at a far higher price than intended. Real run (`water_pump_3`, 2026-07-30): 8 of 12 plan directions read `keep`, the director emitted keep ops of 20/35/7 lines, 62-71% of each source survived and the finished runtime went 22.3min → **53.9min** against a brief asking for 20-30, with `cut` ops down from 21 to 4.

Two changes, one per side of the collision:

- **plan vocabulary renamed.** `PLAN_PROMPT` offers `feature` (not `keep`) in its verb list and JSON example, drops the two mechanical `"keep" preserves …` asides (the plan stage should not describe an op at all — it now says to state in the direction that a part's silent moments matter and leave the mechanics to a later stage), and gains an explicit rule forbidding the word. `DIRECTOR_PROMPT` closes the loop from the other side: a context direction saying a part should be featured/retained/emphasised is editorial emphasis, **not** a request for a keep op.
- **keep op narrowed to its purpose.** It exists to rescue one silent gap — a property of that gap, not of a 35-line span. The prompt now asks for the narrowest range covering the gap (normally `[N, N+1]`) and states that a wide keep adds nothing but dead air (speech is never dropped by default). New `director.max_keep_lines` (default **4**, `0` = no limit) **drops** — never clips — a wider keep at parse time, logged and recorded in the LLM report; dropping degrades to the default behaviour (speech kept, internal silence cut), whereas clipping would restore an arbitrary silence the director never chose. The cap applies to the LLM response only (`try_parse_director_response`); `ops_from_dict` stays uncapped so a human hand-editing `_director.json` can still write a wide keep and mean it. The limit is stated in the system prompt at runtime (`keep_limit_note()`), not hard-coded into `DIRECTOR_PROMPT`, so the number the LLM is told is always the number the parser enforces.

See `docs/superpowers/specs/2026-07-29-keep-semantics-disambiguation-design.md` for the full design (including the accepted residual risk: a dropped wide keep no longer shields its lines from an overlapping `cut` op, which the prompt already forbids) and `tests/director/test_director_llm.py::TestKeepWidthLimit` plus the four new `test_plan_prompt_*`/`test_director_*` tests in `tests/test_config.py` for the pinned guarantees.
