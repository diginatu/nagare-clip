...
A summary says what a neighbouring video is *about*; it never contains the words that neighbour actually ends or begins with. So the director is also handed the transcript at **both of its seams** — the last lines of the video that plays before it and the first lines of the one that plays after, read straight off their `text_filter/{stem}_edits.txt` (`text_filter` has run for every source by the time `director` starts, which is why the *after* side is available at all). `director.seam_lines` (default `3`, `0` = off) sets how many lines per side. Without it every source looks like a standalone episode: in a real 7-source run a full sign-off (`今日は終わりじゃあねー`) played straight into a full greeting (`こんにちはデジナです`) nine minutes into a 20.8-minute video, and the director of the second one had no way to know — it even placed a caption on that greeting. The seam is rendered as text with **no line numbers** — a number there would be the neighbour's coordinate, and ops are always aimed at this video's numbering, so one copied out of the seam would silently edit an unrelated line. The prompt states the rule (your footage is not a standalone episode; those lines belong to another video, carry no numbering, and every op refers to your own numbered lines only) and deliberately shows no worked example of what to do about it — whether to cut the greeting, caption it, or leave it is the director's call. The first video renders only an *after* seam and the last only a *before* one; a missing or unreadable neighbour file degrades to no seam rather than failing, so a single-source re-run still works.

`director.whole_project_context` (default `false`) goes further: every segment's call is handed **the whole finished video's transcript**, in playback order, each segment headed by its label and its *default runtime* (what plays with no op at all — speech at 1x, cut silences dropped) and closed by the whole video's total, so the director can see how long its own stretch is against the whole and what the other segments actually say. The block is rendered once per run by the same renderer as each call's own editable transcript (same brackets, same gap annotations) and sits **inside the cached system-prompt prefix**, so it is byte-identical across every segment's call and costs a cache read after the first. Its lines are numbered `[k]N` (segment `k`, source line `N`) — the parser accepts any number inside a segment's own range, so a bare number copied out of another segment would silently edit an unrelated line here; a one-sentence note says ops use only the plain-numbered transcript in the user message, which now opens with `Edit segment [k] — …`. The caption list is replaced by **the edits already made** to every earlier segment (`[1] stem: timelapse [1]10-[1]85 x18 「…」; cut [1]111-[1]121`), read from this run or, on a `--source` re-run, off disk. With the flag off every request is byte-identical to before.
