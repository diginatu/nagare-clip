...
A director `keep` op is for rescuing **one specific silent gap** that is worth watching, not for marking a long span as important: speech is never dropped by default, so a wide `keep` only restores dead air (in one real run, keep ops of 20-35 lines took a 22.3-minute cut to 53.9 minutes). `director.max_keep_lines` (default `4`, `0` = no limit) drops any wider `keep` the LLM proposes — logged, and listed in the LLM report — so those lines fall back to the normal behaviour (speech kept, internal silence cut). The limit is stated to the director in its prompt automatically, so raising or lowering it needs no prompt edit. It applies only to the LLM's output: a `keep` you write by hand into `{stem}_director.json` is always honoured.
