...
def call_llm(messages: list[dict[str, Any]], cfg: dict[str, Any]) -> str:
    ...
    # Passed through untranslated: what a value means for a given model is
    # LiteLLM's (and the provider's) contract, not this pipeline's. Unset
    # sends nothing, so the model runs at its own default.
