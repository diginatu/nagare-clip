"""Pure frame/interval helpers for the VSE layout (no bpy import — host-testable).

``split_intervals_by_speed`` lives here rather than in ``timeline`` because the
``publish`` stage needs the same source->timeline arithmetic on the host, where
``bpy`` does not exist (``timeline`` imports it at module scope). ``timeline``
re-exports it, so the blender stage's import site is unchanged.
"""
