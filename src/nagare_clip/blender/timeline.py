...
from nagare_clip.blender.frames import clamp_frames, split_intervals_by_speed

# Re-exported so blender_cli (and its tests) keep importing it from here; the
# implementation lives in frames.py because the publish stage needs the same
# arithmetic on the host, where bpy does not exist.
__all__ = ["split_intervals_by_speed"]
