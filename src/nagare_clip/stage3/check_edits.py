"""Stage 3 ``_edits.txt`` integrity checker.

Stage 4 (``sync_text_to_json``) fails fast: it raises a ``ValueError`` on the
*first* segment whose edit line cannot be decomposed against the original
WhisperX JSON, and the keep/speed/overlay extractors only emit log warnings
(without line numbers) when Stage 4 actually runs.  Editing ``_edits.txt`` by
hand is therefore a fix-one-rerun-find-the-next slog.
...
Checks performed (mirroring the real Stage 4 parsing in ``sync_json.py`` so
the verdict matches what Stage 4 would do):
...
# Stage 4 extractors accept.  Used to tokenise a line for balance checking and,
...
def _check_tags(edit_lines: List[str]) -> List[Problem]:
    ...
    allowed, matching the Stage 4 extractors).  ``edit_lines`` must already be
...
def check_edits(
    ...
    # Only the first len(segments) lines are consumed by Stage 4.
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Check a Stage 3 _edits.txt for syntax/integrity problems against "
