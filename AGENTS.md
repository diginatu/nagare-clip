...
Humans may additionally wrap a span in `<keep>...</keep>` to force-preserve the audio under the wrapped text in Stage 4. The tag is added by the human *after* the LLM filter has produced `_edits.txt`; the LLM does not see it.
...
2. Human edits `_cuts.txt` (delete/adjust silent spans) and `_edits.txt` (`{{old->new}}` patch syntax, plus optional `<keep>...</keep>` to force-preserve audio under wrapped text)
3. Resume with `--from-stage 4` → unions cuts, applies patches, syncs JSON, carves out `<keep>` ranges, computes intervals, runs Blender
...
- Stage 4 also extracts `<keep>...</keep>` ranges from `_edits.txt` via `extract_keep_ranges()` in `src/nagare_clip/stage3/sync_json.py`. The time range is `(first wrapped word.start, last wrapped word.end)` from the post-patch synced JSON. Those ranges are subtracted from the unioned excludes (word-gap silence + `_cuts.txt`) via `subtract_intervals()` in `src/nagare_clip/stage3/intervals.py` before merge/invert, so the wrapped audio survives both silence sources. The keep margin config (`intervals.keep_pre_margin` / `keep_post_margin`) does not extend `<keep>` ranges. Empty / unclosed / unmatched / nested tags are skipped with a warning. The `<keep>` marker is added by the human after the LLM filter; the LLM never sees it.
