...
- Stage 4 caption style (font size, alignment, position, shadow, wrap width, outline, box) is controlled by `stage4.caption_style.*` in the config. Options `use_shadow`, `wrap_width`, `use_outline`, `outline_color`, `outline_width`, `use_box`, and `box_color` have no defaults and are only applied when explicitly set.
