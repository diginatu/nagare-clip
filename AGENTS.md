...
- **Inputs:** `{stem}_edits.txt` (from text_filter); optionally the sentence_split `{stem}.json`, passed as `json_path` (for per-line `[dur, gap]` timing), `output/summary/summary.json`, `output/plan/plan.json`, the source stem, passed as `stem`, the gap_context `{stem}_gaps.json`, passed as `gaps`, and the audio_silence `{stem}_cuts.txt`, passed as `cuts_txt` (for per-line speech/silence split), the project's concatenation order, passed as `all_stems`, and the earlier videos' `{stem}_director.json` paths, passed as `prior_director_paths` (for the captions already shown), and the two neighbouring videos' `text_filter/{stem}_edits.txt`, passed as `before_edits`/`after_edits` (for the seam context)
