...
A larger LLM (config `plan:`, disabled by default) runs **once project-wide** after `summary`. It reads the per-part summaries (with stems + line ranges) plus the overall summary and emits a coarse, **cross-video** editorial direction per part (`plan_llm.generate_plan()`, parsing `{"directions":[{"index":N,"direction":...}]}` and mapping each 1-based index back to its part). Cross-video awareness is the point: a part that repeats an earlier video can be marked "remove". The per-part LLM context now also renders each part's **duration + in-between gap** (e.g. `N: stem [a-b] [12.4s, gap 1.5s] — summary`, bracket omitted when times are unavailable), derived from the `start`/`end` stored in `summary.json` — the gap is shown only between consecutive parts of the **same** video (the last part of a video and cross-video boundaries show no gap), and no WhisperX JSON is read at the plan stage. Out-of-range/malformed entries dropped (logged); hard parse failure / LLM error retried via `llm_retry`, then graceful empty. The single `plan.json` (`{directions:[{stem,lines,direction}]}`, self-contained — repeats stem+lines) is human-reviewable and consumed by `director`. Disabled → writes `{"directions":[]}` (no-op).
...
A small local LLM (config `guided_edit:`, disabled by default) applies each director op with **one call over just the op's boundary line(s)** (wide ranges show only the first and last line with an omission marker; the open/close tags on the boundaries span the middle automatically). It inserts `<cut>/<speed>/<overlay>/<keep>` tags (and `{{old->new}}` patches for `edit` ops) at the precise position. `reconcile.verify_op()` then checks, per op, that the underlying transcript text was NOT altered (`clean_old()` strips tags + resolves patches to `old`, compared before/after) and that the op was actually reflected — for a span op (`cut`/`speed`/`overlay`/`keep`) this requires the **opening** tag on the op's first boundary line and the **closing** tag on its last (not merely present somewhere in the range), so the small LLM cannot collapse both tags onto one boundary and silently leave the rest of the span uncut; a failing op is retried (config `guided_edit.max_retries`, default 2) on an LLM error or a failed verification, nudging temperature up each attempt via `nagare_clip.llm_retry.cfg_for_attempt()` (`+retry_temp_step`, capped at `retry_temp_cap`). After all attempts fail the op is reverted and recorded in `{stem}_unapplied.txt` with the last failure reason. Disabled → copies `_edits.txt` through unchanged. A final `check_edits` pass (when `--json` is given) logs any residual problems.
...
- `run_pipeline.sh` accepts `--from-stage S` where `S` is a stage **name** (`transcription`, `audio_silence`, `text_filter`, `summary`, `plan`, `director`, `guided_edit`, `intervals`, `blender`) to skip expensive earlier stages and reuse their outputs; legacy numbers 1-5 still map to transcription/audio_silence/text_filter/intervals/blender. A symmetric `--to-stage S` (same name/legacy values) stops **after** stage `S` (inclusive); later stages are skipped entirely (no output reuse/validation). Default `to_stage` is `blender` (the last stage = no upper bound). Stage execution order is the `STAGE_ORDER` array; each stage runs only inside the `[FROM_ORDER, TO_ORDER]` window (helpers `in_window`/`past_window`). `--from-stage` after `--to-stage` is rejected with an error. Both are also configurable via `pipeline.from_stage` / `pipeline.to_stage` in YAML config. When skipping earlier stages (before `from`), the script validates that required intermediate outputs exist; stages past `to` are simply not run. (`stage_index` returns success even when a name is unknown, so an invalid `--from-stage`/`--to-stage` reaches its friendly error message instead of aborting silently under `set -e`.) Output dirs: legacy `output/stage1`..`output/stage5` for transcription/audio_silence/text_filter/intervals/blender, plus name-only `output/summary/`, `output/plan/`, `output/director/` and `output/guided_edit/`. The intervals stage reads the **guided_edit** `_edits.txt` (which is the text_filter edits passed through when guided_edit is disabled).
...
- `src/nagare_clip/timing.py` is a pure helper module (`segment_times(json_data)` → per-segment `(start, end)` list; `format_dur_gap(dur, gap)` → compact `[4.2s, gap 0.8s]` bracket, gap omitted when there is no next item, whole bracket omitted when duration is unknown, negative gaps clamped to 0.0). It surfaces calculated durations + in-between gaps to the LLMs so they can reason about pacing. The summary and director CLIs each gained a `--json` flag (summary's is repeatable, one WhisperX `{stem}.json` per source) — summary uses it to compute and store per-part `start`/`end` in `summary.json`, director uses it to annotate each numbered-transcript line with `[dur, gap]`. The plan stage reads the precomputed part times from `summary.json` (no JSON access of its own). `run_pipeline.sh` passes `--json "${STAGE1_DIR}/${STEM}.json"` to both the summary and director stages.
